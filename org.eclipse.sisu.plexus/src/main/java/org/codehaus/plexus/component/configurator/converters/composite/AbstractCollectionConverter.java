...
public abstract class AbstractCollectionConverter extends AbstractConfigurationConverter {
    ...
    protected abstract Collection<Object> instantiateCollection(
    ...
    protected final Collection<Object> fromChildren(
    ...
    protected final Collection<Object> fromChildren(
        ...
        for (int i = 0, size = configuration.getChildCount(); i < size; i++) {
            ...
            if (rawChildType != childType && c instanceof ParameterizedConfigurationConverter) {
                ...
                elements.add(pc.fromConfiguration(
                        lookup,
                        xml,
                        rawChildType, //
                        TypeArguments.get(childType),
                        enclosingType, //
                        loader,
                        evaluator,
                        listener));
            } else {
                elements.add(c.fromConfiguration(
                        lookup,
                        xml,
                        rawChildType,
                        enclosingType, //
    ...
    // Maintain binary compatibility with old method signature
    protected final Class<?> getChildType(
            final PlexusConfiguration childConfiguration,
            final Class<?> enclosingType,
            final ClassLoader loader,
            final Class<?> elementType)
            throws ComponentConfigurationException {
    ...
    protected final Type getChildType(
            final PlexusConfiguration childConfiguration,
            final Class<?> enclosingType,
            final ClassLoader loader,
            final Type elementType)
    ...
    protected static final PlexusConfiguration csvToXml(final PlexusConfiguration configuration, final String csv) {
